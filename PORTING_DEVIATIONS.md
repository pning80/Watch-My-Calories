...
| D-012 | T2.x History banner-ad scroll behavior | iOS `HistoryView` keeps `BannerAdView()` inside the ScrollView's VStack — the banner scrolls with content and disappears off the top as the user scrolls through days. | Android `HistoryScreen.kt` places `BannerAdView()` outside the `LazyColumn`, between the `TopAppBar` and the day-card list — the banner stays pinned below the top bar while day cards scroll independently underneath it. | Material idiom favors persistent monetization surfaces above scrolling content; SwiftUI's habit of inlining ads into the scroll list isn't conventional on Android. Banner ad still loads/renders identically; only its scroll-coupling differs. Discovered 2026-05-31 during PR #29 review. | agent | (pending sign-off) |
| D-013 | T2.x Interstitial ads | None. iOS uses banner + native placements only; there is no `GADInterstitialAd` integration anywhere in the iOS source tree. | `AdManager.kt` adds an interstitial flow not present on iOS: `AdManager.loadInterstitial()` is called from the SDK-init callback path and again after each dismissal; `AdManager.showInterstitialIfReady(activity, onDismissed)` shows the ad subject to a 60s cooldown (`INTERSTITIAL_COOLDOWN_MS`) and the `disableForUITesting` guard. Wired into the manual-entry save flow in `MainActivity.MainAppContent` (the `ManualEntryScreen` `onSave` callback calls `AdManager.showInterstitialIfReady`) so saving a manually-entered food item may trigger an interstitial. Uses Google's test unit ID `ADMOB_INTERSTITIAL_ID` until production IDs land via `local.properties` (see PR #30). | Interstitials at natural action-completion moments are a conventional Android monetization pattern and a meaningful fraction of Android in-app revenue; the iOS UX doesn't carry an equivalent SwiftUI idiom. The 60s cooldown + UI-testing guard keep the ad from interrupting frequent saves or breaking instrumentation. Kept as an Android-only enhancement rather than back-porting to iOS (iOS is frozen) or removing the feature. Discovered 2026-05-31 during the AdMob iOS↔Android parity audit. | agent | (pending sign-off) |
