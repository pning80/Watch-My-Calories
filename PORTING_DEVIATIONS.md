...
| D-011 | T2.x Dark-theme palette | iOS uses two-tone dark (`systemBackground` `#000` + `secondarySystemBackground` `#1C1C1E`) with system material layering. The brand mint `#66CC99`, accent `#FF9E1C`, and forest secondary `#264D33` are shared light/dark; text is `label` `#FFFFFF`. | Android dark palette now mirrors iOS for parity (revised 2026-06-01): `background = #000`, `surface = #1C1C1E`, `primary = #66CC99`, `tertiary/accent = #FF9E1C`, text `#FFFFFF` — all matching iOS. The neutral two-tone surfaceContainer ladder (`#000`→`#363638`) replaces the earlier green-tinted one; `surfaceTint = #66CC99` is retained only so any tonal-elevation fallback avoids M3's default purple. **One intentional retained improvement:** `secondary = #B8D5C2` (pale sage) instead of iOS's `#264D33`, which is a too-dark light-theme container value reused in dark mode and reads poorly as a badge fill. Light palette unchanged (already a pixel match). | agent | PN 2026-06-01 (revised: converge to iOS, keep sage secondary) |
