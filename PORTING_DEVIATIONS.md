...
| D-013 | T2.x Interstitial ads | None. iOS uses banner + native placements only; there is no `GADInterstitialAd` integration anywhere in the iOS source tree. | `AdManager.kt` adds an interstitial flow not present on iOS: `loadInterstitial(context)` is called from `initialize()` (line 51) and again after each dismissal; `showInterstitialIfReady(activity, onDismissed)` shows the ad subject to a 60s cooldown (`INTERSTITIAL_COOLDOWN_MS`) and the `disableForUITesting` guard. Wired into the manual-entry save flow at `MainActivity.kt:557` so saving a manually-entered food item may trigger an interstitial. Uses Google's test unit `ca-app-pub-3940256099942544/1033173712` until production IDs land. | Interstitials at natural action-completion moments are a conventional Android monetization pattern and a meaningful fraction of Android in-app revenue; the iOS UX doesn't carry an equivalent SwiftUI idiom. The 60s cooldown + UI-testing guard keep the ad from interrupting frequent saves or breaking instrumentation. Kept as an Android-only enhancement rather than back-porting to iOS (iOS is frozen) or removing the feature. Discovered 2026-05-31 during the AdMob iOS↔Android parity audit. | agent | (pending sign-off) |
