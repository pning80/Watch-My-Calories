...
|---|---|---:|---|---|
| `IOS-BUG-3` | `FoodEntryGroupCard.summaryRow` not reliably tappable via inner StaticText | 5 | Uses `.onTapGesture` (not a Button) — tapping a StaticText child sometimes does not propagate to the parent gesture in XCUITest | **Attempted 2026-05-31 and deferred.** Four approaches tried: `Button { summaryRow }`, `.buttonStyle(.plain)`, `.accessibilityElement(children: .combine) + .accessibilityAddTraits(.isButton)`, `.accessibilityRepresentation { Button(...) }`. **None worked** — `app.buttons["groupCard_summaryRow"]` never reaches the XCUITest a11y tree (`descendants.matching(identifier:).count == 0`). Root cause: SwiftUI consolidation inside the nested `ScrollView → LazyVStack → ForEach → FoodEntryGroupCard → onTapGesture'd row` hierarchy strips child identifiers. **Defers to a structural refactor** (move the tap gesture to the outer VStack instead of inner row) or accepting the 5 tests as architectural limitations. |
| `IOS-BUG-4` | Thumbnail Button has no accessibility label | 2 | `Button(action:) { Image(uiImage:) }` exposes only as an unlabeled Button — `app.images.firstMatch` matches an SF Symbol decorative image (flame.fill, plus.circle.fill, etc.) instead | **Attempted 2026-05-31 and deferred.** Added `.accessibilityIdentifier("foodEntry_thumbnail")` to the inner `Image` element AND `.accessibilityIdentifier("fullScreenImage_closeButton")` to the FullScreenImageView close button. The inner Image identifier is **stripped by the wrapping `.buttonStyle(PlainButtonStyle())`** — same SwiftUI quirk family as IOS-BUG-3. Diagnostic confirmed `app.images["foodEntry_thumbnail"]` never appears in the a11y tree. **Defers to** either dropping `.buttonStyle(PlainButtonStyle())` (visual regression) or restructuring the thumbnail as a `.contentShape(Circle()) + .onTapGesture` pattern with a sibling Text holding the identifier. |
| `IOS-BUG-5` | Manual Entry keyboard focus drops after disclosure expand | 1 | Tapping the DisclosureGroup dismisses the keyboard and a subsequent `.tap()` on the protein field doesn't reliably re-focus | **Fixed 2026-05-31.** Test rewritten to expand the nutrition disclosure FIRST (before any keyboard input), then fill all 4 fields sequentially. Each tap brings the keyboard up cleanly. Pure test-file change, no production code touched. |
