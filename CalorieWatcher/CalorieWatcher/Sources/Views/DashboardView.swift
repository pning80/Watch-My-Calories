    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section(header: Text("Gemini AI Configuration")) {
                    SecureField("API Key", text: $geminiService.apiKey)
                        .onChange(of: geminiService.apiKey) { newValue in
                            Task {
                                await geminiService.fetchModels()
                            }
                        }
                    Link("Get API Key", destination: URL(string: "https://aistudio.google.com/")!)
                    
                    if !geminiService.availableModels.isEmpty {
                        Picker("Model", selection: $geminiService.selectedModel) {
                            ForEach(geminiService.availableModels, id: \.self) { model in
                                Text(model).tag(model)
                            }
                        }
                    } else if !geminiService.apiKey.isEmpty {
                        Text("Fetching models...").font(.caption).foregroundColor(.gray)
                    }
                }
                
                Section {
                    Text("About Calorie Watcher v1.1")
                }
            }
