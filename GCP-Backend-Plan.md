...
- This folder contains both the iOS version and the Andriod version of the Carolie Watcher app. The iOS version is in the folder `CalorieWatcher` and the Android version is in the folder `CalorieWatcherAndroid`.
- Next we will add a backend for the iOS version of the app stored in `CalorieWatcher`. Please ignore the Android version of the app stored in `CalorieWatcherAndroid`. We will refactor the Android version later. Please don't make any changes to the Android version until you receive an explicit instruction to do so.
...
- Let's store the backend code in the folder `Backend`, which should be at the same level as `CalorieWatcher` and `CalorieWatcherAndroid`. 
