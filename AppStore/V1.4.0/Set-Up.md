# App Store Connect Update Setup — Watch My Calories v1.4.0
...
> Scan restaurant menus! Tap the new "Scan Menu" tab to photograph a menu and see AI calorie estimates for every dish. Your approximate location helps identify the restaurant for more accurate results. Scanned menus are saved automatically so you can revisit them anytime.
>
> Also in this update:
> - Redesigned Log Food — tap the "Log Food" tab for a clean sheet with Scan Food, Choose from Library, and Log Manually options
> - New app menu — tap the (...) button to access Settings and About from any screen
> - About screen — view app version, rate on the App Store, and access support and legal links
> - Nutrition hero card — see protein, carbs, and fat breakdown with a proportional bar on your Dashboard
> - Daily nutrition in History — each day card now shows a macro summary when data is available
> - Stored Menus — browse, view, and delete your previously scanned menus
> - Banner ads in sheets — small ads appear in the Log Food and Scan Menu action sheets
...
The v1.4.0 screenshot folder is at `AppStore/V1.4.0/Screenshots/`.
...
| Screenshot | Notes |
|------------|-------|
| Dashboard with hero card | Shows nutrition hero card with macro breakdown and banner ad |
| Log Food sheet | Shows "Scan Food", "Choose from Library", and "Log Manually" options with banner ad |
| Camera scan | Camera view for food scanning |
| AI analysis results | Post-capture food analysis with macro details |
| Scan Menu sheet | Shows "Scan Menu", "Choose from Library", and "Stored Menus" options with banner ad |
| Menu analysis results | Menu items with calorie estimates, expandable macro details |
| Stored Menus | List of previously scanned menus |
| Meal History with nutrition | Past entries by date with daily macro summaries |
| About screen | App version, rate link, support/legal links, attestation status |
| Settings | Profile data, calorie goal, privacy choices |
...
- [ ] **Take screenshots** and add them to the `AppStore/V1.4.0/Screenshots/` folder.
...
> 4. Tap "Scan Food". Grant camera permission. Take a photo of food. Tap "Use". The app analyzes the photo and displays estimated food items, calories, and macros. Tap "Done" to save.
...
Since this is an update, most settings (like Categories and Age Rating) carry over automatically. You only need to complete the following:

- [ ] **What's New in This Version** — Add release notes for v1.4.0. See Section 2.
- [ ] **App Description** — Updated to mention menu scanning, nutrition hero card, stored menus. See Section 2.
...
- [ ] **Upload Build** — Archive and upload V1.4.0 via Xcode. See Section 7.
- [ ] **Select Build** — Choose the uploaded 1.4.0 (build 9) build in App Store Connect.
- [ ] **App Review Notes** — Updated to include menu scanning flow, location permission, new navigation. See Section 5.
- [ ] **Privacy Labels** — Add Coarse Location data type. See Section 4.
- [ ] **Screenshots** — UI changed significantly (new tab layout, Log Food sheet, Scan Menu sheet, menu analysis, stored menus, nutrition hero card, About screen, app menu). See Section 3.

### Optional (If Changed)

- [ ] **Promotional Text** — Updated to mention menu scanning. See Section 2.
- [ ] **Keywords** — Updated to add menu/restaurant/scan. See Section 2.
...
- [ ] **Test app menu** — Tap "..." button. Verify Settings and About appear. Test both.
- [ ] **Test About screen** — Verify version, rate link, support/legal links, and attestation status.
- [ ] **Test nutrition hero card** — Log food with macros. Verify hero card shows protein/carbs/fat proportional bar.
- [ ] **Test history daily nutrition** — Verify history day cards show macro summary when data exists.
