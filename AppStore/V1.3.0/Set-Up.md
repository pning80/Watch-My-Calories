# App Store Connect Update Setup — Watch My Calories v1.3.0
...
> Track your daily calories effortlessly with AI-powered food recognition. Simply snap a photo of your meal — or choose one from your photo library — and Watch My Calories instantly identifies each food item, estimates portions, and calculates calories and macronutrients.
>
> KEY FEATURES
>
...
> Stop guessing and start tracking! Snap a photo or pick one from your library for instant AI calorie estimates. Private, fast, and synced with your fitness goals.
...
The v1.3.0 screenshot folder is at `AppStore/V1.3.0/Screenshots/`.
...
| Add entry sheet | Shows "Scan Food" and "Choose from Photo Library" options |
...
- [ ] **Take screenshots** and add them to the `AppStore/V1.3.0/Screenshots/` folder.
...
*Note: Food photos are sent to Google Gemini AI for analysis. Only the first photo per meal is stored locally on-device. Photos are not stored on the server. Users can also choose photos from their photo library for analysis.*
...
>
> **Photo library access:** Users can also choose a photo from their photo library for analysis. Tap the "+" button on the Dashboard, then select "Choose from Photo Library."
>
...
> 3. Tap the Camera tab → grant camera permission → take a photo of food → tap "Use"
...
> 6. Tap "+" on Dashboard → "Choose from Photo Library" → select a food photo → tap "Use" → review results → tap "Done"
> 7. Tap History tab to view past entries
> 8. Tap Settings tab to see profile data, calorie goal, Device Attestation status, and "Manage Privacy Choices" (visible in GDPR regions)
...
- [ ] **App Description** — Updated to mention photo library feature. See Section 2.
...
- [ ] **Select Build** — Choose the uploaded 1.3.0 (build 4) build in App Store Connect.
...
- [ ] **Promotional Text** — Updated to mention photo library. See Section 2.

### Testing

- [ ] **Test on a physical device** — Verify camera, HealthKit, ad loading, consent sheet, and App Attest all work.
- [ ] **Test photo library flow** — Tap "+" → "Choose from Photo Library" → select a food photo → verify analysis completes and entry saves.
...
- [ ] **Test rate limiting** — Verify the app shows a user-friendly "Too Many Requests" message instead of a generic error.
