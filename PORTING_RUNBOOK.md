...
- **3.8 — Spacing system rollout** (PORT_AUDIT Phase E follow-up). `ui/theme/Spacing.kt` exists with semantic constants (`xs/s/m/l/xl/xxl` + `pageHorizontal`, `cardCorner`, `cardContent`, `cardGap`) and is consumed by `cwCard()` in `ui/components/SharedComponents.kt`. **Partial adoption** — most screens still hardcode `16.dp` / `24.dp` at the call site. Outstanding work is a mechanical sweep replacing hardcoded dp values across `ui/dashboard/`, `ui/history/`, `ui/settings/`, `ui/analysis/`, `ui/menuscanner/`, `ui/photolib/`, `ui/logfood/`, and `ui/onboarding/`. No correctness risk if deferred; the goal is single-sourcing the design tokens so a future spacing change happens in one place.
